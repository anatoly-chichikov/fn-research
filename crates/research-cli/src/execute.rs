...
fn resume(
    ...
    let processor = pend.processor().to_string();
    let language = pend.language().to_string();
    let provider = pend.provider().to_string();
    let exec = (conf.provider)(&provider);
...
fn start(repo: &repository::Repository, session: &ResearchSession, out: &Path, conf: &Config) {
    ...
    let processor = session.processor().to_string();
    let language = session.language().to_string();
    let provider = session.provider().to_string();
    let allowed = ["parallel", "valyu", "xai"];
    if !allowed.contains(&provider.as_str()) {
        panic!("Provider must be parallel valyu or xai");
    }
    let modes = ["fast", "standard", "heavy"];
    if provider == "valyu" && !modes.contains(&processor.as_str()) {
        panic!("Processor must be fast standard or heavy for valyu");
    }
    let exec = (conf.provider)(&provider);
    let run = exec.start(&query, &processor);
...
fn complete(
    ...
    org.response(&name, provider, resp.raw());
    let brief_path = org.brief(&name, provider);
    let brief_ron = brief::encode(content);
    std::fs::write(&brief_path, brief_ron).ok();
    support::store(&name, provider, resp.raw(), out);
    let text = resp.text();
